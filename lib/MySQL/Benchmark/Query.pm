...
=head2 Query Weight (key C<weight>)

The query weight is just an integer. The higher the integer the more a given
query will appear in the workload. The exact proportion will be calculated by
adding up all Query Weight values in the list of queries, and then normalising
it to a value between 1 and 100, in such a way that every query has at least
weight 1 and in total the sum of all normalised weights is 100.
...

{
    ...
    sub __process_query_descriptor {
        ...
        $$self{weight} = $$query_descriptor{weight};
...
Accessor. Returns the weight associated with this query.

If a second value is provided, it will be stored as this instance's weight.

=cut 

sub weight { defined $_[1]? ${ $_[0] }{weight} = $_[1] : ${ $_[0] }{weight} }
